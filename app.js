(function() {
    ...
    function sayTemp(temp, data) {
        var msg = new SpeechSynthesisUtterance("The temperature is " + temp + " at " + data);
        msg.voice = speechSynthesis.getVoices().filter(function(voice) {
            return voice.name == (temp === 'hot') ? 'Deranged' : 'Whisper';
        })[0];
        document.getElementById("tempBody").style.background = (temp === 'hot') ? "red" : "white";
        msg.rate = 1;
        msg.pitch = 2;
    ...
    function hack(data) {
        if (parseInt(data.temp) >= 30) {
            sayTemp("hot", data.temp)
        } else if (parseInt(data.temp) <= 20) {
            sayTemp("cold", data.temp)
        } else {
            document.getElementById("tempBody").style.background = "lavender";
        }
    }
